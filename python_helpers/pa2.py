...
def colorsegms(segm, img):
    ...
    for i in range(1, int(max(segm.flatten())) + 1):
...
def labels2seg(Y,L):
    ...
    rstep = np.floor(L['stepsize']/2.0).astype(int)
    stepbox = range(-rstep, L['stepsize'] - rstep)
    rx = np.asarray(L['rangex'], dtype=int) + L['offset'].astype(int)
    ry = np.asarray(L['rangey'], dtype=int) + L['offset'].astype(int)
